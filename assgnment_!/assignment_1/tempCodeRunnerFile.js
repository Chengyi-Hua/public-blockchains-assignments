...
try {
    ...
    console.log('Asking question, Transaction hash:', tx.hash);
    ...
    if (receipt.status !== 1) {
        console.log('Transaction failed, see more info here: https://sepolia.etherscan.io/tx/' + tx.hash);
        return;
    }

    const { text, id } = extractQuestion(quizContract, receipt);
    console.log("Question is: ", text, "| Question id:", id);
    ...
    // Your code here.
    const txAnswer = await quizContract.answerQuestion(id, userAnswer);
        console.log('Answering question, Transaction hash:', txAnswer.hash);
        const receiptAnswer = await txAnswer.wait();
    ...
    console.log('Stored answer:', storedAnswer, '| Is correct:', isCorrect);    
} catch (error) {
    console.error('Error occurred while getting the answer:', error);
}
